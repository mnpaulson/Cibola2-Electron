...
function setupAutoUpdater() {
  main$1.autoUpdater.on("checking-for-update", () => {
    win == null ? void 0 : win.webContents.send("update-checking");
  });
  ...
  main$1.autoUpdater.on("update-not-available", (info) => {
    win == null ? void 0 : win.webContents.send("update-not-available", {
      version: info.version
    });
  });
  main$1.autoUpdater.on("error", (err) => {
    win == null ? void 0 : win.webContents.send("update-error", err == null ? "unknown" : (err.message || err).toString());
  });
  main$1.autoUpdater.on("download-progress", (progressObj) => {
    win == null ? void 0 : win.webContents.send("update-download-progress", {
...
const settingsPath = require$$1$1.join(app.getPath("userData"), "settings.json");
function readSettings() {
  try {
    if (require$$1.existsSync(settingsPath)) {
      const data = require$$1.readFileSync(settingsPath, "utf-8");
...
function writeSettings(settings) {
  try {
    require$$1.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), "utf-8");
    return true;
  } catch (err) {
...
function createWindow() {
  win = new BrowserWindow({
    icon: require$$1$1.join(process.env.VITE_PUBLIC, "256x256.png"),
    width: 1280,
    height: 800,
    webPreferences: {
  ...
  win.on("app-command", (e, cmd) => {
    if (cmd === "browser-backward") {
      win == null ? void 0 : win.webContents.send("app-navigate-back");
    } else if (cmd === "browser-forward") {
      win == null ? void 0 : win.webContents.send("app-navigate-forward");
    }
  });
...
ipcMain.handle("download-update", async () => {
  try {
    const result = await main$1.autoUpdater.downloadUpdate();
    return { success: true, result };
  } catch (err) {
